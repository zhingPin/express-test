...
interface SendTransactionArgs {
  ...
  value?: string; // ETH to send (in Ether, not Wei)
  data?: `0x${string}`; // Contract interaction data
  nonce?: number; // Transaction nonce
  gasPrice?: string; // Gas price in Gwei
  accessList?: AccessList; // EIP-2930 access list
  factoryDeps?: `0x${string}`[]; // Bytecodes for factory dependencies
  paymaster?: Address; // Paymaster address
  paymasterInput?: `0x${string}`; // Paymaster input
}
