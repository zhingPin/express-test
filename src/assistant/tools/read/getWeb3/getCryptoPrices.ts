...
interface GetCryptoPriceArgs {
  symbol: string; // Cryptocurrency symbol (e.g., BTC, ETH)
  currency: string; // Currency to convert (e.g., USD, EUR)
}

export const getCryptoPriceTool: ToolConfig<GetCryptoPriceArgs> = {
  definition: {
    type: "function",
    function: {
      name: "get_crypto_prices",
      description:
        "Get the current price of a cryptocurrency in a given currency using CoinMarketCap",
      parameters: {
        type: "object",
        properties: {
          symbol: {
          ...
          currency: {
            type: "string",
            description:
              "The currency to convert the cryptocurrency into (e.g., USD, GBP)",
          },
        },
...
async function getCryptoPrice({ symbol, currency }: GetCryptoPriceArgs) {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Error fetching data: ${response.statusText}, Details: ${errorDetails}`
    );
  }
  ...
  console.log(
    "API response data:",
    JSON.stringify(data.data[symbol][0]?.quote, null, 2)
  ); // Format for easier inspection

  // Check if the response has data for the symbol
  if (!data.data[symbol]) {
    throw new Error(
      `Symbol '${symbol}' not found in the response data. Response data: ${JSON.stringify(
        data.data
      )}`
    );
  }
  ...
  if (!Array.isArray(data.data[symbol])) {
    throw new Error(
      `Symbol '${symbol}' data is not in array format. Data: ${JSON.stringify(
        data.data[symbol]
      )}`
    );
  }
  ...
  if (data.data[symbol].length === 0) {
    throw new Error(
      `No data found for symbol '${symbol}'. Response: ${JSON.stringify(
        data.data[symbol]
      )}`
    );
  }
  ...
  if (!quoteData) {
    throw new Error(
      `Currency '${currency}' not found for symbol '${symbol}' in the response. Response: ${JSON.stringify(
        data.data[symbol][0]
      )}`
    );
  }
  ...
  if (!price) {
    throw new Error(
      `Price for symbol '${symbol}' in '${currency}' not available. Response: ${JSON.stringify(
        quoteData
      )}`
    );
  }
