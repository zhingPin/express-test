...
interface GetBalanceArgs {
  wallet: Address;
  chain: ChainKeys; // Added chain parameter as string
}
...
//  */
export const getTokenBalanceTool: ToolConfig<GetBalanceArgs> = {
  definition: {
    type: "function",
    function: {
      name: "get_balance",
  ...
  handler: async ({ wallet, chain }) => {
    return await getBalance(wallet, chain as ChainKey); // Cast `chain` to ChainKey
  },
};
...
//  */
async function getBalance(wallet: Address, chainKey: ChainKey) {
  try {
    ...
    if (error instanceof Error) {
      console.log(
        `Error fetching balance for wallet ${wallet} on chain ${chainKey}:`,
        error
      );
