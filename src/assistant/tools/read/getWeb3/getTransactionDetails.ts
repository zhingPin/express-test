...
import { getPublicClient } from "../../../../utils/clients/web3/viem/helpers/viemUtils.js";
...
export const getTransactionDetailsTool: ToolConfig<GetTransactionDetailsArgs> =
{
  definition: {
    type: "function",
    function: {
      ...
      description:
        "Get the details of a transaction including `from`, `to`, and `hash`",
      parameters: {
        type: "object",
        properties: {
          hash: {
          ...
          chain: {
            type: "string",
            description:
              "The chain to use for fetching the transaction details (required)",
          },
        },
        required: ["hash", "chain"],
      },
...
async function getTransactionDetails(hash: Hash, chain: ChainKey) {
  try {
    // Get the first client from the public clients array
    const publicClient = getPublicClient(chain);

    console.log(
      `Fetching transaction details for hash: ${hash} on chain: ${chain}`
    );
