...
export const dexPriceAggregatorTool: ToolConfig<TokenPriceQueryArgs> = {
  definition: {
    type: "function",
    function: {
...
const getDexPoolsData = async ({
  ...
  try {
    ...
    const filteredPools = data.data.pools.filter((pool: any) => {
      return (
        symbols.includes(pool.token0.symbol) ||
