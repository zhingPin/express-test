...
const getThreadandLatestRunStatus = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
...
const getAssistantChunk = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (existingMessage) {
        ...
        let textContent = "";
        if (Array.isArray(existingMessage.content)) {
            const firstPart = existingMessage.content[0];
            if (firstPart.type === "text" && firstPart.text?.value) {
                textContent = firstPart.text.value;
            }
        }
