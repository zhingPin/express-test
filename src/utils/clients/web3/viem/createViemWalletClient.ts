...
import { ChainKey, ChainKeys, viemChains } from "./viemChains.js";
...
//  */
export function createViemWalletClient(chainKey: ChainKey) {
  const chainConfig = viemChains[chainKey];
  if (!chainConfig) {
    throw new Error(`Unsupported chain: ${chainKey}`);
  }

  const privateKey = process.env.NEO_PRIVATE_KEY as `0x${string}`;
  if (!privateKey) {
