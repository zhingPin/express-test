 ...
 */
export async function getGitRepoPullRequests(
  owner: string,
  repo: string,
  fields: string[] = []
): Promise<any> {
  try {
    ...
    for (const pr of data) {
      ...
      if (fields.includes("id")) prData.id = pr.id;
      if (fields.includes("title")) prData.title = pr.title;
      if (fields.includes("state")) prData.state = pr.state;
      if (fields.includes("createdAt")) prData.createdAt = pr.created_at;
      if (fields.includes("updatedAt")) prData.updatedAt = pr.updated_at;
      if (fields.includes("user")) prData.user = pr.user.login;
