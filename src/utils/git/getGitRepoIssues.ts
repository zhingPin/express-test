import { createGitClient } from "../clients/createGitClient.js";
 ...
 */
export async function getGitRepoIssues(
  owner: string,
  repo: string,
  fields: string[] = []
): Promise<any> {
  try {
    ...
    for (const issue of data) {
      ...
      if (fields.includes("id")) issueData.id = issue.id;
      if (fields.includes("title")) issueData.title = issue.title;
      if (fields.includes("state")) issueData.state = issue.state;
      if (fields.includes("createdAt")) issueData.createdAt = issue.created_at;
      if (fields.includes("updatedAt")) issueData.updatedAt = issue.updated_at;
