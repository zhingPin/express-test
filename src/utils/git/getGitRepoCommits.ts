 ...
 */
export async function getGitRepoCommits(
  owner: string,
  repo: string,
  fields: string[] = []
): Promise<any> {
  try {
    ...
    for (const commit of data) {
      ...
      if (fields.includes("sha")) commitData.sha = commit.sha;
      if (fields.includes("author"))
        commitData.author = commit.commit.author.name;
      if (fields.includes("date")) commitData.date = commit.commit.author.date;
