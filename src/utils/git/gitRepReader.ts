...
import { GitRepoMetadata } from "../../types/git-types.js";
 ...
 */
export async function getGitRepoMetadata(
  owner: string,
  repo: string,
  fields: string[] = []
  ...
  try {
    ...
    if (fields.includes("name")) metadata.name = data.name;
    if (fields.includes("owner")) metadata.owner = data.owner.login;
    if (fields.includes("description")) metadata.description = data.description;
    ...
    if (fields.includes("createdAt")) metadata.createdAt = data.created_at;
    if (fields.includes("updatedAt")) metadata.updatedAt = data.updated_at;
    if (fields.includes("latestCommit")) metadata.latestCommit = data.pushed_at;
