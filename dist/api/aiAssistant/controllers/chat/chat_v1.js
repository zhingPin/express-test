...
const getThreadRunStatus2 = catchAsync(async (req, res, next) => {
    ...
    const updatedThread = await ThreadModel.findByIdAndUpdate(req.params.id, {
        run: {
            id: latestRun.id,
...
const getAssistantResponse2 = catchAsync(async (req, res, next) => {
            ...
            content: [
                {
                    type: "text",
                    text: {
                        value: "⚠️ The assistant was unable to generate a response due to an internal error. Please try again later.",
                        annotations: [],
                    },
                },
            ],
    ...
    console.log("🔄 Cached response not found. Creating response:existingMessage.");
...
const createMessage2 = catchAsync(async (req, res, next) => {
    ...
    if (thread.run && thread.run.status === "requires_action") {
        console.log(`🔄 cm Run already in progress (${thread.run.status}), skipping run creation.`);
        return next();
    }
...
const createRun2 = catchAsync(async (req, res, next) => {
    ...
    if (thread.run && thread.run.status === "requires_action") {
        console.log(`🔄 cr Run already in progress (${thread.run.status}) for (${thread.run.id}), skipping perform run.`);
        const run = await client.beta.threads.runs.retrieve(thread.threadId, thread.run.id);
        console.log(`🔄 cr retrieved runData (${run.status}) for (${run.id}), skipping perform run.`);
        // req.run = run;
    ...
    const updatedThread = await ThreadModel.findByIdAndUpdate(thread._id, {
        run: {
            id: newRun.id,
...
const performRun2 = catchAsync(async (req, res, next) => {
    ...
    if (run.status === "in_progress" || run.status === "queued") {
        console.log(`⏳ Run still in progress (${run.status}), polling will continue...`);
        res.status(200).json({
            status: "success",
            data: {
                run,
