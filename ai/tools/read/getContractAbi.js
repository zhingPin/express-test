...
export const getContractAbiTool = {
  definition: {
    ...
    function: {
      ...
      description:
        "Get the ABI or specific function signature of a deployed contract on a specific blockchain network",
      parameters: {
        type: "object",
        properties: {
          chain: {
...
async function getContractAbi(chain, contract, functionName) {
  try {
    ...
    return `Error fetching ABI from chain ${chain}: ${
...
function extractFunctionSignatures(abiString, functionName) {
  try {
    ...
    if (functionName) {
      ...
      const exact = functions.find((f) =>
        f.toLowerCase().startsWith(`${functionName.toLowerCase()}(`)
      );
      if (exact) return exact;
      // Try partial match
      const partial = functions.find((f) =>
